...
pub struct RaydiumAmmDex {
    pub base_pools: Vec<Arc<dyn DexPoolInterface>>,
}
...
impl RaydiumAmmGrpcSubscriber {
        ...
        for (index, chunks) in snapshot_accounts.chunks(100).enumerate() {
            ...
            let handle = tokio_spawn(format!("fetch_snapshot_{}", index).as_str(), async move {
                ...
                match account_fetch_result {
                    ...
                    Err(e) => {
                        error!("raydium amm dex fetch account snaphot error,{}", e)
                    }
                };
                snapshot_pool
            });
    ...
    pub async fn subscribe(
        ...
        let raydium_amm_need_subscribe_pools = RaydiumAmmGrpcSubscriber::get_snapshot(
            rpc_url.to_string(),
            dex.clone(),
        ...
        .await;
        let dex_name = dex.name();
        tokio_spawn(format!("{} grpc sub", dex_name).as_str(), async move {
            let mut sub_pool_accounts: HashMap<String, SubscribeRequestFilterAccounts> =
                HashMap::new();
            sub_pool_accounts.insert(
                format!("{}-sub", dex_name),
                SubscribeRequestFilterAccounts {
                    account: raydium_amm_need_subscribe_pools
                        .iter()
                        .map(|(pool_id, _, _)| pool_id.to_string())
                        .collect::<Vec<_>>(),
                    ..Default::default()
                },
            );
            let pool_subscribe_request = SubscribeRequest {
                accounts: sub_pool_accounts,
                commitment: Some(CommitmentLevel::Processed).map(|x| x as i32),
                accounts_data_slice: PoolUpdate::subscribe_request_data_slices(),
                ..Default::default()
            };
            let mint_vault_subscribe_request = MintVaultSubscribe::create_subscribe_request(
                raydium_amm_need_subscribe_pools,
                None,
            );
            ...
            let (mut trigger_event_holder, mut clear_timeout_update_cache) =
                TriggerEventHolder::new_holder_with_expired_interval(None, None, None);
            ...
            info!("【RaydiumAMM】GRPC subscribe finished");
            loop {
                tokio::select! {
                        ...
                        if let Some(message_type) = data {
                            match message_type {
                                SourceMessage::GrpcAccountUpdate(account_type, account_info,timestamp,filter_name) => {
                                    ...
                                    if let Some(event)= amm_trigger_event{
                                        if let Some(event)=trigger_event_holder.fetch_event(Box::new(event)){
                                            info!("【RaydiumAMM】EventHolder触发ruote, txn : {:?} , account : {:?}, data : {:?}",event.get_txn(),event.get_pool_id(),event);
                                            if let Err(e) = trigger_event_sender.send(event){
                                                error!("【RaydiumAMM】EventHolder触发ruote失败, 原因: {}",e);
                                            }
                                        }
                                    }
                                }
...
impl From<(Pubkey, Vec<u8>)> for PoolUpdate {
    fn from(value: (Pubkey, Vec<u8>)) -> Self {
        let src = array_ref![value.1.as_slice(), 0, 80];
        let (
            // swap_fee_numerator,
    ...
    fn update_and_return_ready_event(
            ...
            && self.mint_1_vault_update.is_some()
        {
            Some(Box::new(Self {
                ...
                pool_update: self.pool_update,
                mint_0_vault_update: self.mint_0_vault_update,
                mint_1_vault_update: self.mint_1_vault_update,
