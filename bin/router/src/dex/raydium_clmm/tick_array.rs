use crate::dex::raydium_clmm::pool::REWARD_NUM;
...
impl TickArrayState {
    ...
    pub fn from_slice_data(
        ...
        let start_tick_index =
            i32::from_le_bytes(dynamic_slice_data[32..32 + 4].try_into().unwrap());
        let mut ticks: [TickState; TICK_ARRAY_SIZE_USIZE] = [TickState::default(); 60];
        let ticks_slice_data = &dynamic_slice_data[36..36 + 60 * (4 + 16 + 16)];
        for (index, tick_data) in ticks_slice_data.chunks(4 + 16 + 16).enumerate() {
            ticks[index] = TickState::from_slice_data(tick_data);
        }
        let initialized_tick_count = u8::from_le_bytes(
            dynamic_slice_data[36 + 60 * (4 + 16 + 16)..36 + 60 * (4 + 16 + 16) + 1]
                .try_into()
                .unwrap(),
        );
        Self {
...
impl TickState {
    ...
    pub fn from_slice_data(slice_data: &[u8]) -> Self {
        let tick = i32::from_le_bytes(slice_data[0..4].try_into().unwrap());
        let liquidity_net = i128::from_le_bytes(slice_data[4..4 + 16].try_into().unwrap());
        let liquidity_gross = u128::from_le_bytes(slice_data[20..20 + 16].try_into().unwrap());
