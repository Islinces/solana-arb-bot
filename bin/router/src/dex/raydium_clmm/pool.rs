use crate::dex::raydium_clmm::big_num::U1024;
use crate::dex::raydium_clmm::tick_array::TickArrayState;
use crate::dex::raydium_clmm::tick_array_bit_map::check_current_tick_array_is_initialized;
use crate::dex::raydium_clmm::tickarray_bitmap_extension::TickArrayBitmapExtension;
use crate::dex::raydium_clmm::{tick_array_bit_map, tick_math};
...
impl PoolState {
    ...
    pub fn key(&self) -> Pubkey {
        Pubkey::create_program_address(&self.seeds(), DexType::RaydiumCLMM.get_ref_program_id())
            .unwrap()
    }
    ...
    pub fn from_slice_data(static_slice_data: &[u8], dynamic_slice_data: &[u8]) -> Self {
        let amm_config = Pubkey::try_from(&static_slice_data[0..32 * 1]).unwrap();
        let token_mint_0 = Pubkey::try_from(&static_slice_data[32..32 * 2]).unwrap();
        let token_mint_1 = Pubkey::try_from(&static_slice_data[32 * 2..32 * 3]).unwrap();
        let token_vault_0 = Pubkey::try_from(&static_slice_data[32 * 3..32 * 4]).unwrap();
        let token_vault_1 = Pubkey::try_from(&static_slice_data[32 * 4..32 * 5]).unwrap();
        let observation_key = Pubkey::try_from(&static_slice_data[32 * 5..32 * 6]).unwrap();
        let tick_spacing =
            u16::from_le_bytes(static_slice_data[32 * 6..32 * 6 + 2].try_into().unwrap());
        let liquidity = u128::from_le_bytes(dynamic_slice_data[0..16].try_into().unwrap());
        let sqrt_price_x64 =
            u128::from_le_bytes(dynamic_slice_data[16..16 + 16].try_into().unwrap());
        let tick_current =
            i32::from_le_bytes(dynamic_slice_data[16 + 16..16 + 16 + 4].try_into().unwrap());
        let mut tick_array_bitmap = [0; 16];
        for (index, data) in dynamic_slice_data[16 + 16 + 4..16 + 16 + 4 + 8 * 16]
            .chunks(8)
            .into_iter()
            .enumerate()
        {
            tick_array_bitmap[index] = u64::from_le_bytes(data.try_into().unwrap());
        }
        Self {
            amm_config,
            token_mint_0,
