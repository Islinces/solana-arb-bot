...
use crate::defi::raydium_clmm::sdk::big_num::U128;
...
/// the square root of the ratio of the two assets (token_1/token_0) (y/x)开根
///
...
pub fn get_sqrt_price_at_tick(tick: i32) -> Result<u128, anchor_lang::error::Error> {
    ...
    let mut ratio = if abs_tick & 0x1 != 0 {
        // 1111111111111100101110010011001110111101011011111011100000000000
    ...
    // i = 1
    if abs_tick & 0x2 != 0 {
        // 相乘之后可能会超过u128的范围，右移64位重新调整到U64.64范围内
        // 这些魔法因子都是 2^64 / (1.0001^(2^(i - 1)))，预定义好，快速计算出结果
        // 可以理解为将(1.0001^(2^(i - 1)))左移了64位，用于在计算中提高精度，最后再右移64位恢复回来
        // i 最大 18 是一个合理的值（防止值越界，精度也满足，较大的i对于结果的影响越小）
        // 1111111111111001011100100111001000110111001111010100000000000000
