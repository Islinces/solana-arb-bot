...
use crate::dex::meteora_damm_v2::constants::{BASIS_POINT_MAX, ONE_Q64};
...
pub fn get_fee_in_period(
    ...
    let result = pow(base, passed_period.into()).ok_or_else(|| anyhow!(MathOverflow))?;

    let (fee, _) = result
        .safe_mul(cliff_fee_numerator.into())?
        .overflowing_shr(SCALE_OFFSET);

    let fee_numerator = u64::try_from(fee).map_err(|_| anyhow!(TypeCastFailed))?;
    Ok(fee_numerator)
}
